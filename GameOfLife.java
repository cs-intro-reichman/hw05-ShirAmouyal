...
return board;
	...
	public static int[][] evolve(int[][] board) {
		...
		for(int i=1 ; i<board.length-1 ; i++){
           for(int k=1 ; k<board[0].length-1 ; k++){
            NewBoard[i][k]=cellValue(board, i, k);
		   }
		}
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		if(board[i][j]==1){ // Any live
		 ...
		 if(board[i][j] == 0){ //Any dead cell with exactly three live neighbors becomes a live cell.
